...
export function MembersScreen(this: any) {
  ...
  const { state, trigger, celulaId, setTrigger, setCelulaId, dispatch } =
    useFormReport();
  ...
  const redesUnicas =
    redes &&
    redes.filter(function (este: any, i: any) {
      return redes.indexOf(este) === i && este;
    });

  const mapRedesUnicas =
    redesUnicas &&
    redesUnicas.map((item: any) => {
      return {
        value: item,
      };
    });
  ...
  const discipulado =
    filtrandoRedes &&
    filtrandoRedes.map((item: any) => item[1].discipulador?.trim());

  const discipuladossUnicos =
    discipulado &&
    discipulado.filter(function (este: any, i: any) {
      return discipulado.indexOf(este) === i;
    });

  const mapDiscipuladosUnicos =
    discipuladossUnicos &&
    discipuladossUnicos.map((item: any) => {
      return {
        value: item,
      };
    });

  const filtrandoDiscipulado =
    celulas &&
    celulas.length > 0 &&
    celulas?.filter((item: any) => {
      if(item[1]){
      return (
        item[1].discipulador === state.discipuladoSelect &&
  ...
  const celulaAdm =
    filtrandoDiscipulado &&
    filtrandoDiscipulado.map((item: any) => {
      return {
  ...
  const filtrandoDiscipuladoPastor =
    celulas &&
    celulas.length > 0 &&
    celulas?.filter((item: any) => {
      if(item[1]){
  ...
  const mapDiscipuladoPastor =
    filtrandoDiscipuladoPastor &&
    filtrandoDiscipuladoPastor.map((item: any) => {
      return item[1].discipulador;
    });

  const discipuladossUnicosPastor =
    mapDiscipuladoPastor &&
    mapDiscipuladoPastor.filter(function (este: any, i: any) {
      return mapDiscipuladoPastor.indexOf(este) === i;
    });

  const mapDiscipuladossUnicosPastor =
    discipuladossUnicosPastor &&
    discipuladossUnicosPastor.map((item: any) => {
      return {
        value: item,
      };
    });
  ...
  const filtrandoDiscipuladoPastorSelect =
    celulas &&
    celulas.length > 0 &&
    celulas?.filter((item: any) => {
      if(item[1]){
  ...
  const celulaPastor =
    filtrandoDiscipuladoPastorSelect &&
    filtrandoDiscipuladoPastorSelect.map((item: any) => {
      return {
        value: `${item[1].numero_celula} - ${item[1].lider}`,
      };
    });
                          ...
                          <PersonLabelComponent
                            ...
                            onPress={() =>
                              navigation.navigate("MemberInformation", {
                                ...
                                data_de_nascimento:
                                  !item[1].data_de_nascimento ||
                                  item[1].data_de_nascimento.includes("/")
                                    ? item[1].data_de_nascimento
                                    : `${format(
                                        new Date(item[1].data_de_nascimento),
                                        "dd/MM/yyyy"
                                      )}`,
